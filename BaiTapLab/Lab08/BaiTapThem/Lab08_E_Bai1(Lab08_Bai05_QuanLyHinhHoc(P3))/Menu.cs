...
namespace Lab08_E_Bai1_Lab08_Bai05_QuanLyHinhHoc_P3__
{
    public class Menu
    {
        ...
        private void Show()
        {
            ...
            Console.WriteLine("{0}. To mau hinh hoc", (int)ChucNang.ToMauHinhHoc);
        ...
        private void Process(ChucNang menu)
        {
            switch (menu)
            {
                ...
                case ChucNang.TinhDoChenhLechNhoNhatVeDienTich:
                    ...
                    break;
                case ChucNang.ToMauHinhHoc:
                    Console.WriteLine("Nhap loai hinh muon to mau (1: HinhTron, 2: HinhVuong, 3: HinhChuNhat):");
                    int loaiHinhToMau = int.Parse(Console.ReadLine());
                    Console.WriteLine("Nhap mau (VD: Red, Blue, Green):");
                    string mau = Console.ReadLine();
                    Color color = Color.FromName(mau);
                    bool found = false;

                    switch (loaiHinhToMau)
                    {
                        case 1: // Hinh Tron
                            var hinhTrons = danhSachHinhHoc.GetHinhTron();
                            foreach (var shape in hinhTrons)
                            {
                                if (shape is IMauSac hinhCoMau)
                                {
                                    hinhCoMau.ToMau(color);
                                    Console.WriteLine($"Da to mau cho hinh tron: {shape}");
                                    found = true;
                                }
                            }
                            break;
                        case 2: // Hinh Vuong
                            var hinhVuongs = danhSachHinhHoc.GetHinhVuong();
                            foreach (var shape in hinhVuongs)
                            {
                                if (shape is IMauSac hinhCoMau)
                                {
                                    hinhCoMau.ToMau(color);
                                    Console.WriteLine($"Da to mau cho hinh vuong: {shape}");
                                    found = true;
                                }
                            }
                            break;
                        case 3: // Hinh Chu Nhat
                            var hinhChuNhats = danhSachHinhHoc.GetHinhChuNhat();
                            foreach (var shape in hinhChuNhats)
                            {
                                if (shape is IMauSac hinhCoMau)
                                {
                                    hinhCoMau.ToMau(color);
                                    Console.WriteLine($"Da to mau cho hinh chu nhat: {shape}");
                                    found = true;
                                }
                            }
                            break;
                    ...
                    if (!found)
                    {
                        Console.WriteLine("Khong tim thay hinh phu hop hoac hinh khong ho tro to mau.");
                    }
                    break;
                case ChucNang.Thoat:
